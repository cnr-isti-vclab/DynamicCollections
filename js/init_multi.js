...
function init3dhop() {
	...
	jQuery('.tdhop:not(.tdcanvas)').on('contextmenu', function(e) { return false; });
...
function resizeCanvas(id,w,h) {
  jQuery('#draw-canvas'+id).attr('width', w);
  jQuery('#draw-canvas'+id).attr('height',h);
  jQuery('#3dhop'+id).css('width', w);
  jQuery('#3dhop'+id).css('height', h);
}
